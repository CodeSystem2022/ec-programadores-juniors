...
const ProductView = () => {
    ...
    useEffect(() => {
        Axios.get(`${import.meta.env.VITE_APP_HOST}/product/product/${id}`).then(response => {
    ...
    return (
        <FullWidthLayout>
            <div className="product-container">
                ...
                <div className="size">
                    {
                        product != null ?
                            product.sizes.map((size, index) => (
                                <button onClick={() => { setSelectedSize(size) }} className="size-button" key={index}>{size}</button>
                            ))
                            :
                            <h1>Loading</h1>
                    }
                ...
                <button onClick={() => handleClick(product)} className="add-button">Agregar al carrito</button>
